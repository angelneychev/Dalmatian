...
C#, ASP.NET Core 3.1, Entity Framework Core 3.1, SQL Server, ASP.NET Core Identity, Razor, AutoMapper, JavaScript, jQuery, Bootstrap, Font Awesome, Cloudinary and SendGrid. The solution also includes xUnit test projects.
...
This is a historical .NET Core 3.1 application. The following instructions describe the checked-in configuration; a clean installation has not been verified as part of the documentation update.

You need a development environment capable of building and running `netcoreapp3.1`, NuGet access and a separate local SQL Server database. Visual Studio can open `src/Dalmatian.sln`.
...
This checkout contains implemented club workflows alongside unfinished parts. Treat it as a historical codebase to review and run locally, with deployment readiness still to be assessed.
